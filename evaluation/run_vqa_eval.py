...
def load_inference_module(model_name: str) -> Callable:
    ...
    model_file = Path(__file__).parent / "inference_models" / f"{model_name}.py"
...
def extract_image_id(item: Dict[str, Any], dataset: str) -> Any:
    ...
    if dataset in ["vqav2_val", "vqav2_restval"]:
...
def save_results(data: List[Dict], model_name: str, dataset_name: str, output_dir: str = "./results/") -> None:
